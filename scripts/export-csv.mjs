...
// Columns: kind (word | stem | phrase | suffix | leet), word (Latin form), devanagari (paired spelling or "").
import { writeFileSync } from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { lexicon } from "../dist/index.js";

// Mapping dictionary for Latin-to-Devanagari pairings.
// Extended to cover all new terms added in the updated lexicon.
const PAIRED = {
  // English (Latin only)
...
// Process Latin words with their paired Devanagari translation if available
for (const w of lexicon.LATIN_WORDS) push("word", w, PAIRED[w] ?? "");

// Process standalone Devanagari words that aren't already paired in the dict
